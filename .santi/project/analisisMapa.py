        ...
        if jerarquia[i][3] == -1:
            ...
            if len(approx) == 3 and area >= 500 and area < 3000:  # Triángulo
                ...
                center_x, center_y = (
                    x + w // 2,
                    y + h // 2,
                )  # Centro aproximado del triángulo
                row = center_y // cell_height
                col = center_x // cell_width
                cell_index = row * cols + col  # Índice de la celda
...
if not cap.isOpened():
    ...
    cv2.createTrackbar(
        "Canny Th1", "Ajustes", canny_threshold1, 255, on_trackbar_change
    )
    cv2.createTrackbar(
        "Canny Th2", "Ajustes", canny_threshold2, 255, on_trackbar_change
    )
    ...
    while True:
        ...
        detected_shapes, frame_with_shapes = detect_shapes_in_image(
            frame, rows, cols, threshold1, threshold2, dilatacion
        )

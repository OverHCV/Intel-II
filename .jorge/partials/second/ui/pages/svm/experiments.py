...
def render_experiment_history(session_state_svm):
    ...
    """
    if "experiment_history" not in session_state_svm or not session_state_svm["experiment_history"]:
        return
...
def _find_best_experiment(history):
    ...
    if "CV Accuracy" in history[0]["metrics"]:
        best_idx = max(range(len(history)), key=lambda i: history[i]["metrics"]["CV Accuracy"])
        best_acc = history[best_idx]["metrics"]["CV Accuracy"]
    elif "Accuracy" in history[0]["metrics"]:
        best_idx = max(range(len(history)), key=lambda i: history[i]["metrics"]["Accuracy"])
...
def _render_comparison_chart(history, best_idx):
    ...
    ax.set_title(f'{metric_to_plot} Comparison Across Experiments', fontsize=14, fontweight='bold')
